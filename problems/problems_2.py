...
class Pallindrome1: # Focus on what matters, order does not
    def pallindrome1(self, i: int) -> bool:
        if i < 0:
            return False
        stringY = i[::-1] # Can also move them aorund with m% 10 to get individual digits, but that's cumbersome
        if i == stringY:
            print("True")
            return True
        else:
...
testInstance = Pallindrome()
testInstance.pallindrome(i = 1231)

class maxSubarray: # Remember, we are not returning the aray, we are returning the RESULT
    def subArray(self, ns: list[int]) -> int:
        max = ns[0] # We need one, so we can start with the first
        temp = ns[0]
        ...
        for num in ns:
            ...
            if (temp > max):
                max = temp
            if (temp < 0):
                temp = 0 # We reset temp here so we don't carry extra negatives. However, we will still be chekcing if they're larger than the previous ones
            return max